...
function url($url, $params=array(), $redirect=false, $suffix=true) {
    ...
    if (config('URL_MODEL') > 0) {
        $depr = config('URL_PATHINFO_MODEL') == 2 ? config('URL_PATHINFO_DEPR') : '/';
        $str = $depr;
        ...
        if ($suffix && config('URL_HTML_SUFFIX'))
            $url .= config('URL_HTML_SUFFIX');
    }else {
        $params = http_build_query($params);
        $params = !empty($params) ? '&' . $params : '';
        if (isset($group)) {
            $url = str_replace(APP_NAME, $app, __APP__) . '?' . config('VAR_GROUP') . '=' . $group . '&' . config('VAR_MODULE') . '=' . $module . '&' . config('VAR_ACTION') . '=' . $action . $params;
        } else {
            $url = str_replace(APP_NAME, $app, __APP__) . '?' . config('VAR_MODULE') . '=' . $module . '&' . config('VAR_ACTION') . '=' . $action . $params;
        }
    }
    if (isset($sub_domain)) {
...
function __autoload($name) {
    ...
    if (substr($name, -5) == "Model") {
        require_cache(LIB_PATH . 'Model/' . $name . '.php');
    } elseif (substr($name, -6) == "Action") {
        require_cache(LIB_PATH . 'Action/' . $name . '.php');
    } else {
        // 根据自动加载路径设置进行尝试搜索
...
function import($class, $baseUrl = '', $ext='.class.php') {
    ...
    if ($ext == '.class.php' && is_file($classfile)) {
        ...
        if (isset($_class[$class]))
            throw_exception(lang('_CLASS_CONFLICT_') . ':' . $_class[$class] . ' ' . $classfile);
...
// 远程调用模块的操作方法
function action($module, $action, $app='@') {
    $class = controller($module, $app);
    if ($class)

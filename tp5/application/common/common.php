...
/*--------------------------------------------------系统常用快捷函数开始-----------------------------------------------------------------*
// 返回数组的值
function array($array, $key=0){
	return $array[$key];
}
// 递归多维数组转为一级数组
function arrays2array($array){
...
/*-------------------------------------------------系统采集函数开始------------------------------------------------------------------*
// 采集内核
function file_get_contents($url, $timeout=10, $referer=''){
...
// 采集-匹配规则结果all
function preg_match_all($rule,$html){
	$arr = explode('$$$',$rule);
	if(count($arr) == 2){
...
//TAG分词自动获取
function tag_auto($title,$content){
	$data = file_get_contents('http://cdn.feifeicms.co/server/v3/kw.php?source='.rawurlencode(msubstr($title.$content,0,200)));
	if($data) {
...
// 格式化采集影片主演
function xml_vodactor($vodactor){
	return str_replace(array('/','，','|','、',' ',',,,'),',',$vodactor);
}
/*-----------------------------------------------------------模板标签解析函数开始-----------------------------------------------------------------*
//获得某天前的时间戳
function linux_time($day){
	$day = intval($day);
...
// 获取时间颜色
function color_date($type='Y-m-d H:i:s',$time,$color='red'){
	if((time()-$time)>86400){
...
function page_count($pageid='pageid', $key='records'){
	// 通过GET全局变量获当前定义的
	$page = $_GET['page_'.$pageid];
	if(!$page){
		return false;
	}
	return $page[$key];
...
// 处理最大分页参数
function page_max($currentpage, $totalpages){
...
// 获取与处理人气值
function get_hits($sidname, $type='hits', $array, $js=true){
...
function detail_array($module='vod', $type='next', $id, $cid, $field='vod_id,vod_cid,vod_status,vod_name,vod_ename,vod_jumpurl'){
	...
	$array = D(ucfirst($module))->find($field, $where, 'cache_page_'.$module.$type.'_'.$id, false, $order);
	return $array;
}
//播放列表格式化
function play_list($server, $play, $url){
	//加载配置文件
	...
	foreach($conf_play as $conf_key=>$conf_value){
		if($play_one = $array[$conf_key]){
			foreach($play_one as $sid=>$url_one){
				$son = play_list_one($url_one, $conf_server[$array_server[$sid]]);
...
//分解单组播放地址链接
function play_list_one($url_one, $server_url){
	$url_list = array();
...
//获取某一组的播放地址
function play_one($vod_play_list, $play_name='max'){
	if($play_name == 'max'){
...
function url_replace_route($model, $params, $url){
...
//静态模式 格式化页码为1的首个链接
function url_replace_html($model, $link){
	$replace = false;
	if( $model=='news/show' ){
		if(C('url_news_list')){
			$replace = true;
...
// 根据后台设置的静态网页规则生成保存路径
function url_build($module, $params){
...
// 共用分类页链接函数
function url_show($model='vod/show', $params, $suffix=true){
	...
	if(C('url_html')){
		...
		if( $model=='news/show' ){
			if(C('url_news_list')){
				$url = url_build('news/show', $params).C('html_file_suffix');
				return url_replace_html($model, $url);
			}
		}else if( $model=='vod/show' ){
			if(C('url_vod_list')){
				$url = url_build('vod/show', $params).C('html_file_suffix');
				return url_replace_html($model, $url);
			}
		}
	}
	...
	if(C('URL_ROUTER_ON')){
		if($params['p'] == 'FFLINK'){
			$params['p'] = 0.20161212;
			return str_replace('0.20161212', 'FFLINK', url($model, $params, $suffix));
		}
	}
	return url($model, $params, $suffix);
}
// 共用内容页链接函数
function url_detail($model='vod/read', $params, $suffix=true){
	...
	if(C('url_html')){
		if( $model=='news/read' ){
			if(C('url_html') && C('url_news_detail')){
				$url = url_build('news/read', $params).C('html_file_suffix');
				return url_replace_html($model, $url);
			}
		}else if( $model=='vod/read' ){
			if(C('url_html') && C('url_vod_detail')){
				$url = url_build('vod/read', $params).C('html_file_suffix');
				return url_replace_html($model, $url);
			}
		}else if($model=='vod/play'){
			if(C('url_html') && C('url_vod_play')){
				$url = url_build('vod/play', $params).C('html_file_suffix');
				return url_replace_html($model, $url);
			}
		}
	}
...
//动态链接函数 url('news/read',array('id'=>3,'p'=>2),true);
function url($model, $params, $suffix=true){
	//是否存在跳转链接
	if ($params['jumpurl']) {
		return $params['jumpurl'];
	}
	...
	// 自定义路由反向生成对应的URL
	if( C('URL_ROUTER_ON') ){
		$url = url_replace_route($model, $params, $url);
	}
	// 伪静态
	if(C('url_rewrite')){
...
//分页带链接展示函数
function url_page($model, $params, $suffix=false, $pageid='pageid', $halfPer=5, $page = false){
	if(!$page){
		$page = $_GET['page_'.$pageid];
	}
	if(!$page){
	...
	$jumpurl = url_show($model, $params, $suffix);
	//
	...
	for($i=$page['currentpage']-$halfPer,$i>1||$i=1,$j=$page['currentpage']+$halfPer,$j<$page['totalpages']||$j=$page['totalpages'];$i<$j+1;$i++){
		...
		if($i == 1){
			$params['p'] = 1;
			$firrt_page = url_show($model, $params, $suffix);
			if($page['currentpage']==1){
...
// 视频分类页链接函数
function url_vod_show($list_id,$list_dir,$list_page,$suffix=true){
	return url_show('vod/show', array('id'=>$list_id,'list_dir'=>$list_dir,'p'=>$list_page), $suffix);
}
// 文章分类页链接函数
function url_news_show($list_id,$list_dir,$list_page,$suffix=true){
	return url_show('news/show', array('id'=>$list_id,'list_dir'=>$list_dir,'p'=>$list_page), $suffix);
}
// 视频详情页链接函数
function url_vod_read($list_id,$list_dir,$vod_id,$vod_ename,$vod_jumpurl,$suffix=true){
	return url_detail('vod/read',array('list_id'=>$list_id,'list_dir'=>$list_dir,'id'=>$vod_id,'pinyin'=>$vod_ename,'jumpurl'=>$vod_jumpurl), $suffix);
}
// 视频播放页链接函数
function url_vod_play($list_id,$list_dir,$vod_id,$vod_ename,$play_sid,$play_pid,$suffix=true){
	return url_detail('vod/play',array('list_id'=>$list_id,'list_dir'=>$list_dir,'id'=>$vod_id,'pinyin'=>$vod_ename,'sid'=>$play_sid,'pid'=>$play_pid),$suffix);
}
// 文章详情页链接函数
function url_news_read($list_id,$list_dir,$news_id,$news_ename,$news_jumpurl,$news_page=1,$suffix=true){
	return url_detail('news/read',array('list_id'=>$list_id,'list_dir'=>$list_dir,'id'=>$news_id,'pinyin'=>$news_ename,'jumpurl'=>$news_jumpurl,'p'=>$news_page), $suffix);
}
// 获取影片最后一集 return array(sid,pid,jiname,jipath,pic,info)
function url_play_end($vod_url){
	$arr_url = array();
...
function url_search($str, $type="actor", $sidname='vod', $action='search'){
  ...
  $str = str_replace(array('/','|',',','，'),',',$str);
	...
	foreach($arr as $key=>$val){
		$array[$key] = '<a href="'.url($sidname.'/'.$action, array($type=>urlencode($val)), true).'">'.$val.'</a>';
	}
	return implode('',$array);
}
// Tag链接
function url_tags($str, $tag_list='vod_tag'){
	list($module,$type) = explode('_',$tag_list);
	return url($module.'/tags', array('name' => urlencode($str) ), true);
}
// 内容页Tag链接
function url_tags_content($content, $array_tag){
	if($array_tag){
		foreach($array_tag as $key=>$value){
			if($value['tag_name']){
				$keyword = stripslashes($value['tag_name']);
				$url = '<a href="'.url_tags($value['tag_name'], $value['tag_list']).'" class="ff-tag-link" >'.$keyword.'</a>';
				$regEx = '\'(?!((<.*?)|(<a.*?)))('. $keyword . ')(?!(([^<>]*?)>)|([^>]*?</a>))\'s';
...
// 获取某图片的访问地址
function url_img($file, $content, $number=1){
	if(!$file){
		return url_img_preg($content, $number);
	}
...
// 获取某图片的缩略图地址
function url_img_small($file, $content, $number=1){
	if(!$file){
		return url_img_preg($content, $number);
	}
	$array = parse_url($file);
...
//正则提取正文里指定的第几张图片地址
function url_img_preg($content, $number=1, $ext='gif|jpg|jpeg|bmp|png'){
...
// 获取26个字母链接
function url_letters($file='vod',$str=''){
	if(C('url_html')){
		$index='index.html';
	}else{
		$index='index.php';
    ...
    for($i=1;$i<=26;$i++){
	   $url = url($file.'/search', array('id'=>chr($i+64),'x'=>'letter'), true);
...
/*-------------------------------------------------栏目分类相关函数开始------------------------------------------------------------------*
//通过栏目条件获取对应的栏目名称/别名等
function list_find($cid, $field='list_name'){
	$info = D("List")->find($field, array('list_id'=>array('eq',$cid)), false);
	if($info){
		return $info[$field];
	}else{
		return false;
...
//获取当前分类的子类
function list_ids($cid){
	$tree = list_search(mysql_list('limit:0;cahce_name:default;cahce_time:default;order:list_oid;sort:asc'), 'list_id='.$cid);
	$array = array();
...
// 获取栏目数据统计
function list_count($cid){
	$where = array();
	if(999 == $cid){
		$rs = M("Vod");
		$where['vod_cid'] = array('gt',0);
		$where['vod_addtime'] = array('gt',linux_time(1));//当天更新的影视
		...
		if ($sid == '2'){
			...
			$where['vod_cid'] = array('in',list_ids($cid));
			$where['vod_status'] = 1;
			$rs = M("Vod");
			$count = $rs->where($where)->count('vod_id');
		}
...
function param_url(){
	...
	$array['order'] = order_by($_GET['order']);
...
function mysql_nav($tag){
	...
	return D('Nav')->select_page(mysql_param($tag), $where);
}
// 循环标签.轮播
function mysql_slide($tag){
	$tag = param_lable($tag);
	$where = array();
	...
	return D('Slide')->select_page(mysql_param($tag), $where);
}
// 循环标签.分类
function mysql_list($tag){
	$tag = param_lable($tag);
	$where = array();
	...
	return D('List')->select_page(mysql_param($tag), $where);
}
// 循环标签.友链
function mysql_link($tag){
	$tag = param_lable($tag);
	$where = array();
	if ($tag['ids']) {
	...
	return D('Link')->select_page(mysql_param($tag), $where);
}
// 循环标签.TAG话题
function mysql_tags($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	if ($tag['ids']) {
	...
	$rs = D('Tag');
	return $rs->select_page(mysql_param($tag), $where);
}
// 循环标签.视频
function mysql_vod($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	...
	if ($tag['upday']) {
		$where['vod_addtime'] = array('gt',linux_time($tag['upday']));
	}
	if ($tag['lastday']) {
		$where['vod_hits_lasttime'] = array('gt',linux_time($tag['lastday']));
	}
	if (isset($tag['filmday'])) {
		$where['vod_filmtime'] = array('gt',linux_time($tag['filmday']));
	}
...
function mysql_news($tag_str){
	...
	if ($tag['day']) {
		$where['news_addtime'] = array('gt',linux_time($tag['day']));
	}
	if (isset($tag['lastday'])) {
		$where['news_hits_lasttime'] = array('gt',linux_time($tag['lastday']));
	}
	if ($tag['hits']) {
		$hits = explode(',',$tag['hits']);
		if (count($hits) > 1) {
...
function mysql_forum($tag_str){
	...
	if ($tag['upday']) {
		$where['forum_addtime'] = array('gt',linux_time($tag['upday']));
	}
	if ($tag['wd']) {
		$search = array();
	...
	return D('ForumView')->select_page(mysql_param($tag), $where, $viewFields);
}
// 循环标签.用户资料
function mysql_user($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	if (isset($tag['status'])) {
	...
	if ($tag['addtime']) {
		$where['user_addtime'] = array('gt',linux_time($tag['addtime']));
	}
	if ($tag['logtime']) {
		$where['user_logtime'] = array('gt',linux_time($tag['logtime']));
	}
	if ($tag['wd']) {
		$search = array();
		$search['user_name'] = array('like','%'.$tag['wd'].'%');
	...
	return D('User')->select_page(mysql_param($tag), $where);
}
// 循环标签.订单
function mysql_orders($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	if (isset($tag['status'])) {
	...
	if ($tag['addtime']) {
		$where['order_addtime'] = array('gt',linux_time($tag['addtime']));
	}
	if ($tag['paytime']) {
		$where['order_paytime'] = array('gt',linux_time($tag['paytime']));
	}
	if ($tag['confirmtime']) {
		$where['order_confirmtime'] = array('gt',linux_time($tag['confirmtime']));
	}
	if ($tag['wd']) {
		$search = array();
	...
	return D('OrdersView')->select_page(mysql_param($tag), $where);
}
// 循环标签.记录
function mysql_record($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	if ($tag['ids']) {
	...
	if ($tag['upday']) {
		$where['record_addtime'] = array('gt',linux_time($tag['upday']));
	}
	// 分支加载不同的关联条件
	if($tag['sid'] == 1){
		$viewFields = array (
	...
	return D('RecordView')->select_page(mysql_param($tag), $where, $viewFields);
}
// 循环标签.记录
function mysql_score($tag_str){
	$tag = param_lable($tag_str);
	$where = array();
	//根据参数生成查询条件
	if ($tag['ids']) {
	...
	if ($tag['addtime']) {
		$where['score_addtime'] = array('gt',linux_time($tag['addtime']));
	}
	// 分支加载不同的关联条件
	if($tag['sid'] == 1){
		$viewFields = array (
	...
	return D('ScoreView')->select_page(mysql_param($tag), $where, $viewFields);
}
/*---------------------------------------ThinkPhp扩展函数库开始------------------------------------------------------------------
 * @category   Think
 * @package  Common

...
class Cj extends Model {
	...
	public function jump($page, $pagemax, $pagelink){
		if($page < $pagemax){
			//缓存断点续采并跳转到下一页
			$jumpurl = str_replace('LINK',($page+1), $pagelink);
			cache('_cj/xucai',$jumpurl);
			echo '<meta http-equiv="refresh" content='.config('collect_time').';url='.$jumpurl.'>';
			echo '<h5>'.config('collect_time').'秒后将自动采集下一页!</h5>';
		}else{
			//清除断点续采
			cache('_cj/xucai',NULL);
	...
	private function vod_json($admin, $params){
		...
		//不是bibibcms的格式
		if(!$json['list']){
			return array('status'=>602, 'type'=>'json', 'infos'=>'不是bibibcms系统的接口，不支持采集。');
		}
		//返回正确的数据集合
	...
	public function vod_update($admin, $params, $json){
		...
		if($admin['field']){
			...
			foreach($json['data'] as $key=>$vod){
				...
				if($status){
					echo '<li>重采['.bb_list_find($vod['vod_cid']).'] '.$vod['vod_name'].' '.$array_url['field'].'更新完成。</li>';
				}else{
					echo '<li>重采['.bb_list_find($vod['vod_cid']).'] '.$vod['vod_name'].' 跳过、不做任何处理。</li>';
				}
				ob_flush();flush();
			}
		}else{
			foreach($json['data'] as $key=>$vod){
				echo '<li>第<span>'.(($admin['page']-1)*$json['page']['pagesize']+$key+1).'</span>个影片 ['.bb_list_find($vod['vod_cid']).'] '.$vod['vod_name'].'</li>';
	...
	private function vod_db_find($vod){
		...
		if($array){
			...
			//演员有部份相同时更新该影片
			$arr_actor_1 = explode(',', bb_xml_vodactor($vod['vod_actor']));
			$arr_actor_2 = explode(',', bb_xml_vodactor($array['vod_actor']));
	...
	private function vod_db_insert($vod){
		...
		if(empty($vod['vod_ename'])){
			$vod['vod_ename'] = bb_pinyin($vod['vod_name']);
		}
		if(empty($vod['vod_up'])){
		...
		$vod['vod_letter'] 	= bb_url_letter($vod['vod_name']);
		// 随机伪原创
		if(C('collect_original')){
			$vod['vod_content'] = bb_rand_str($vod['vod_content']);
		}
		// 自动下载远程图片
		...
		if($id){
			...
			if(empty($vod['vod_keywords']) && C('collect_tags')){
				$vod['vod_keywords'] = bb_tag_auto($vod["vod_name"], $vod["vod_content"]);
			}
			// 增加关联tag
			if( $vod['vod_keywords'] ){
		...
		return '视频添加失败：'. db('Vod')->getDbError();
	}
	//根据影片ID更新数据
	private function vod_db_update($vod, $vod_old){
		// 检测是否站长手动锁定更新
		if('bibibcms' == $vod_old['vod_inputer']){
			return '站长已锁定该影片，不更新。';
		}
		// 检测播放地址是否变化
		...
		// 更新数据
		db('Vod')->data($edit)->save();
		//删除数据缓存
		if(C('cache_page_vod')){
			cache(md5('cache_page_vod_'.$vod_old['vod_id']),NULL);
		}
	...
	private function vod_db_field($vod, $reurl){
		if($vod['vod_pic']){
			$vod['vod_pic'] = model('Img')->down_load($vod['vod_pic']);
		}
		return db('Vod')->where("vod_inputer !='bibibcms' and vod_reurl='".$reurl."'")->data($vod)->save();
	}
	...
	private function vod_array2url($play_list_merge){
		...
		//只添加已定义的播放器
		$array_player = F('_bibibcms/player');
	...
	private function vod_xml_caiji($admin, $params){
		...
		$xml_detail = bb_file_get_contents($url_detail);
		if(!$xml_detail){
			return array('status'=>601, 'infos'=>'连接API资源库失败，通常为服务器网络不稳定或禁用了采集。');
		}
		...
		if($admin['action'] == 'show'){
			$xml = simplexml_load_string(bb_file_get_contents($url_list));
			$key = 0;
	...
	private function user_db($user){
		...
		$array = db('User')->field('user_id,user_name')->where('user_name="'.$user['user_name'].'"')->find();
		if(!$array){
			$user['user_pwd'] = md5(time());
			$user['user_jointime'] = time();
			$user['user_logtime'] = time();
			$user['user_logip'] = get_client_ip();
			$id = db('User')->data($user)->add();
			if($id){
				echo('<li class="p">用户添加成功，'.$user['user_name'].'（'.$id.'）</li>');
			}else{
				echo('<li class="p">用户添加失败，'. db('User')->getDbError().'</li>');
			}
		}else{
	...
	public function forum_db($forum){
		...
		$array = db('Forum')->field('forum_id')->where('forum_referer="'.$forum['forum_referer'].'"')->find();
		if($array){
			echo('<li class="p red">评论已存在，不做处理!</li>');
			return false;
		}
		$vod= db('Vod')->field('vod_id')->where('vod_reurl="'.$forum['vod_reurl'].'"')->find();
		if($vod){
			...
			if($id){
				...
				echo('<li class="p red">评论添加失败，'. db('Forum')->getDbError().'</li>');
			}
		}else{
	...
	private function scenario_db($scenario){
		...
		if($scenario['vod_reurl']){
			$array = db('Vod')->field('vod_id,vod_name,vod_inputer,vod_scenario')->where('vod_reurl="'.$scenario['vod_reurl'].'"')->find();
		}else{
			$array = db('Vod')->field('vod_id,vod_name,vod_inputer,vod_scenario')->where('vod_name="'.$scenario['vod_name'].'"')->find();
		}
		// 检测是否站长锁定
		if('bibibcms' == $array['vod_inputer']){
	...
	public function news_db($news){
	  if(empty($news['news_name']) || empty($news['news_content'])){
		...
		$array = db('News')->field($field)->where('news_cid='.$news['news_cid'].' and news_name="'.$news['news_name'].'" ')->find();
		if($array){
		...
		if( empty($news['news_ename']) ){
			$news['news_ename'] = bb_pinyin($news['news_name']);
		}
		...
		// 入库接口
		$id = db('News')->data($news)->add();
		// 关联多分类及TAG相关
		if($id){
			// 增加多分类
			if( $news['news_type'] ){
				model('Tag')->tag_update($id, $news["news_type"], 'news_type');
			}
			// 增加关联tag
			if( $news['news_keywords'] ){
				model('Tag')->tag_update($id, $news["news_keywords"], 'news_tag');
			}
	...
	private function news_db_update($news, $news_old){
		// 检测是否站长手动锁定更新
		if('bibibcms' == $news_old['news_inputer']){

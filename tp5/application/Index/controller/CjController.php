...
class CjController extends Controller {
	...
	public function show() {
		...
		if ($admin['cj_type'] == 1) {
			$this->display('/cj_list_vod');
		} elseif ($admin['cj_type'] == 2) {
			$this->display('/cj_list_forum');
		} elseif ($admin['cj_type'] == 3) {
			$this->display('/cj_list_scenario');
		} else {
			$this->display('/cj_list');
		}
	}
	// 添加编辑采集节点
	public function add() {
	...
	public function insert() {
		...
		if ($rs->create()) {
			if (false !== $rs->add()) {
				$this->assign("jumpUrl", '?s=Cj-Show-type-' . intval($_POST['cj_type']));
